function [ maxDeltaTpx ] = fIdCOSTplusP2( varOtm, M, argumentos )
global iter
...
%Preprocessamento:
otimizacao = argumentos.simulacao;
opcao_sign = argumentos.opcao_sign;
opcaoSignAt = argumentos.opcaoSignAt;
%{
opcao_m2 = 1;
opcao_sign = 2;
opcaoSignAt = 1;
...
switch otimizacao
    case 1
        alfaTeta = argumentos.alfaTeta; %RPM/s
        alfaTeta = alfaTeta*(pi/30); %rad/s2
        wMax = argumentos.wMax; %RPM
    ...
    case 1
        ...
        for i = 2:nT
            ...
            if (tnM1 - dt/2 < TempoEns(iOtm,1)) && (TempoEns(iOtm,1) <= tnM1 + dt/2)
                tOtm = TempoEns(iOtm,1)
                ubpEFnM1 = Auru*uTilpTxnM1 + AtXu*fTetapX1(tnM1);
                tpXNM1 = ubpEFnM1(Ng,1);
                if abs(tpXNM1 - TetapXNM1enc(iOtm,1)) > maxDeltaTpx
                    maxDeltaTpx = abs(tpXNM1 - TetapXNM1enc(iOtm,1));
                end
                iOtm = iOtm + 1;
            end
        end
        plot(iter,maxDeltaTpx,'ko')
        xlabel('iterações')
        ylabel('custo')
        grid
        iter = iter + 1;
